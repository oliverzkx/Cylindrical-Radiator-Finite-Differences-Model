...
extern "C" void launch_cuda_heat(float* host_prev, int n, int m, int p,
...
int main(int argc, char* argv[]) {
    ...
    if (use_cpu) {
        ...
        cpu_time_ms = std::chrono::duration<float, std::milli>(t1 - t0).count();
    ...
    gpu_time_ms = std::chrono::duration<float, std::milli>(t1_gpu - t0_gpu).count();
